...
# 전처리에 필요한 정보 리스트 생성
    ...
    max_lst = stock['High'].rolling(window=235).max()
    min_lst = stock['Low'].rolling(window=235).min()
...
# 데이터 프레임 정렬 ( 스케일링 필요한 24개앞으로 )
    ...
    scaled_stock_lst =[]
# 스케일링 과정
    for i, (stock_row, date, max_value, min_value) in enumerate(zip(stock_lst, date_lst, max_lst, min_lst)):
        volume = stock_row[24]
        ...
        for value in stock_row[:24]:
            scaled_inform.append( (value-min_value)/(max_value-min_value) )
...
# 10일치 데이터 컬럼으로 스택 과정
    ...
    merging_data = []
    for ii, (row, date, label_value, change) in enumerate(zip(scaled_stock_lst, date_lst, label_lst, change_lst)):
        if date <= start + pd.DateOffset(years=1) or np.isnan(label_value) : # next change가 nan이면 드롭
            continue
        trading_value = row[25]

        # 거래 대금이 10억 이상 5% 이상 변동이 있는 날
